...
- **Phase 1 & 3 sorting instructions:** **`PHASE13_CLICK_ENTER_INSTRUCTION`** in **`context_shape_task.py`** — gray trial hint plus **`phase1_instr3`** / **`phase3_instruction2c`** during **interactive sorting** (verbatim **`script.md`** — unchanged).
- **Animated tutorial fallback** (when **`tutorial_video.mp4`** is missing or playback fails): **`PHASE13_CLICK_ENTER_INSTRUCTION`** appears **once**, as the on-screen subtitle during **`step=2`** (square placement demo); **`step=3`** and **`step=4`** use grouping narrative subtitles only; **no** duplicate full-screen recap after **`step=5b`** (TTL **`trial_info`** **`step=`** **`1` … `5b`** — **`csv_documentation.md`**).
...
| 2 | **6** s (`TUTORIAL_FB_CLICK_CENTER_SEC` + `TUTORIAL_FB_CLICK_TARGET_SEC`; each constant includes **`TRAINING_DEMO_SCREEN_EXTRA_SEC`**) | Red square; **`PHASE13_CLICK_ENTER_INSTRUCTION`** subtitle (**animated fallback only**, **`trial_info` `step=2`**, sole demo appearance) |
...
- The task can take 15-45 seconds to load up.
...
- Once you run it in Python/Terminal, make sure not to click anywhere else on the screen: this might prevent the keyboard from working as expected. If so, quit and start again. 
